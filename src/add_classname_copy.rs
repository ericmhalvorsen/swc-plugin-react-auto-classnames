...
#[derive(Default)]
pub struct AddClassnameVisitor;

impl VisitMut for AddClassnameVisitor {
...
fn process_var_declarator(var_decl: &mut VarDeclarator) -> Option<Component> {
    ...
    let is_jsx = IsJSXVisitor::test(var_decl);
    if !is_jsx { return None };
...
fn process_fn_expr(fn_expr: &mut FnExpr) -> Option<Component> {
    let is_jsx = IsJSXVisitor::test(fn_expr);
    if !is_jsx { return None };
...
fn process_fn_decl(fn_decl: &mut FnDecl) -> Option<Component> {
    let is_jsx = IsJSXVisitor::test(fn_decl);
    if !is_jsx { return None };
